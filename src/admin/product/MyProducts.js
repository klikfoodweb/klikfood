...
class MyProducts extends Component {
	...
	componentDidMount() {
		axios.get(`https://api.klikfood.id/index.php/mitra/produk/`+sessionStorage.id, { 'headers': { 'Authorization': sessionStorage.api_token } })
		  .then((response) => {
	...
	promoLayout(cell, row){
		...
		if(row.promo !== 1) {
			return <button onClick={ (e) => {
	  			...
	  			axios.get(`https://api.klikfood.id/index.php/mitra/produk/${id}/1`)
	  		      .then(res => {
			...
			return <button onClick={ (e) => {
	  			...
	  			axios.get(`https://api.klikfood.id/index.php/mitra/produk/${id}/0`)
	  		      .then(res => {
