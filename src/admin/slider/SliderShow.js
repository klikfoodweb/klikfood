...
class SliderShow extends Component {
	...
	componentWillMount() {
		...
		axios.get(`http://apiklikfood.herokuapp.com/slider/show/`+this.props.match.params.id)
	  .then((response) => {
	...
	handleSubmit = (event) => {
		...
		axios.post(`http://apiklikfood.herokuapp.com/slider/update/`+this.props.match.params.id, bodyFormData)
	      .then(response => {
	      	this.setState({
				submitting: true
			})
