...
class PickCourier extends Component {
	constructor(props) {
		...
		cartNya.map( (item, i) => {
			axios.get(`http://apiklikfood.herokuapp.com/produksupplyer/show/`+item[0], { 'headers': { 'Authorization': sessionStorage.api_token } })
		      .then((response) => {
	...
	handleSubmit = (e) => {
		...
		axios.post(`http://apiklikfood.herokuapp.com/distribusi/store`, obj)
	      .then(res => {
	      	console.log(res);
