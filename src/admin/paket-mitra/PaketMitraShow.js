...
class PaketMitraShow extends Component {
	...
	handleSubmit = (e) => {
		...
		axios.patch(`https://api.klikfood.id/index.php/paketmitra/update/`+this.props.match.params.id+"?"+qs.stringify(obj, {encode: false}))
	      .then(res => {
	      	console.log(res);
	...
	componentWillMount() {
		axios.get(`https://api.klikfood.id/index.php/produksupplyer/all?type=verify`, { 'headers': { 'Authorization': sessionStorage.api_token } })
		 ...
		 //  	let initiateItem = [...Array(150)].map( x => Array(2).fill(0) );
		  ...
		  axios.get(`https://api.klikfood.id/index.php/paketmitra/show/`+this.props.match.params.id)
		  .then((response) => {
		  	console.log(response.data.data);
		  	this.setState({
